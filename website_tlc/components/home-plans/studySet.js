...
const StudySetPage = ({ planCode }) => {
        ...
        getHomePlan('all').then((data) => {
            if (data) {
                ...
                if (homePlan && homePlan.planCode) {
                    ...
                    let sp1 = homePlan && !isNaN(homePlan.studySetCost) ? Number(homePlan.studySetCost) : 0;
    ...
    const handleSubmit = async (event) => {
        ...
        let payload = {
            ...
            //OP = ORDER PLAN
            "OP_HomePlanCd1": selectedPlan1 && selectedPlan1.planCode ? selectedPlan1.planCode : '',
            "OP_HomePlanCd2": selectedPlan2 && selectedPlan2.planCode ? selectedPlan2.planCode : '',
            "OP_HomePlanCd3": selectedPlan3 && selectedPlan3.planCode ? selectedPlan3.planCode : '',
            "OP_Price1": selectedPlan1 && selectedPlan1.studySetCost ? selectedPlan1.studySetCost : '',
            "OP_Price2": selectedPlan2 && selectedPlan2.studySetCost ? selectedPlan2.studySetCost : '',
            "OP_Price3": selectedPlan3 && selectedPlan3.studySetCost ? selectedPlan3.studySetCost : '',
            "OP_PriceTotal": total,
            //CC = CREDIT CARD
    ...
    const handleSelectedPlan = (e, planCd) => {
        ...
        if (planCd === "plan_2") {
            ...
            if (homePlan && homePlan.planCode) {
                ...
                let sp2 = homePlan && !isNaN(homePlan.studySetCost) ? Number(homePlan.studySetCost) : 0;
        ...
        if (planCd === "plan_3") {
            ...
            if (homePlan && homePlan.planCode) {
                ...
                let sp3 = homePlan && !isNaN(homePlan.studySetCost) ? Number(homePlan.studySetCost) : 0;
    ...
    */
    ...
    const getUserDetails = () => {
        ...
        setFormData({
            ...formData,
            ay_firstName: firstName,
            ay_lastName: lastName,
            ay_emailAddress: email,
            ay_phone: phone
        });
    }
    ...
    */
    ...
    const setUserDetails = () => {
        localStorage.setItem('user', JSON.stringify({
            first_name: formData.ay_firstName,
            last_name: formData.ay_lastName,
            email: formData.ay_emailAddress,
            phone: formData.ay_phone
        }));
    }
    ...
    return (
        <>
            <Head>
                <title>{`${planCode}`} Study Set by The Log Connection</title>
                <meta property="og:image" content={`${IMG_BANNER_FILE_ROOT}${planCode}.jpg`} />
                <meta property="og:type" content="article" />
                <meta property="og:url" content={`${HOME_PLAN_DETAILS_PATH.STUDY_SET}/${planCode}`} />
                <meta property="og:title" content={`${planCode} Study Set by The Log Connection`} />
                <meta property="og:description" content={`Study Set by The Log Connection for ${planCode}`} />
            </Head>
            <HomePlanBanner img={`${IMG_BANNER_FILE_ROOT}banner.jpg`} />
            <FloaterNav homePlans={allHomePlans} context="study-set" />
                ...
                <MDBContainer>
                    <MDBRow center className={`${STYLES.formCont} p-3`}>
                            ...
                            <form autoComplete="off" onSubmit={handleSubmit}>
                                ...
                                <section id="about_you">
                                    ...
                                    <MDBRow center className={STYLES.formText}>
                                        <MDBCol md="2" sm="12" className="mb-2">
                                ...
                                <section id="shipping_address">
                                    ...
                                    <MDBRow center className={STYLES.formText}>
                                        ...
                                        <MDBCol md="4" sm="12" className={`${STYLES.selectCont}`}>
                                ...
                                <section id="project_info">
                                    <p className={STYLES.formSectionTitle}>PROJECT INFORMATION <small>(OPTIONAL)</small></p>
                                    <MDBRow center className={STYLES.formText}>
                                        <MDBCol md="2" sm="12">
                                            <label htmlFor="pi_country">COUNTRY</label>
                                        </MDBCol>
                                        <MDBCol md="4" sm="12" className={`${STYLES.selectCont}`}>
                                            <select id="pi_country" name="pi_country" onChange={setCountryStateMappingForProjectInfo}>
                                                <option value={''}>--Select Country--</option>
                                                {COUNTRIES.map((country, index) => (
                                                    <option value={country.name} key={index}>{country.name}</option>
                                                ))}
                                        ...
                                        <MDBCol md="4" sm="12">
                                            <DatePicker selected={buildDate} onChange={(date) => setBuildDate(date)} className={STYLES.inputBox} />
                                        </MDBCol>
                                    </MDBRow>
                                    <MDBRow center className={STYLES.formText}>
                                        <MDBCol md="2" sm="12">
                                        ...
                                        <MDBCol md="4" sm="12" className={`${STYLES.selectCont}`}>
                                            <select id="pi_state" name="pi_state" onChange={handleInput}>
                                                <option value={''}>--Select State--</option>
                                        ...
                                        <MDBCol md="2" sm="12">
                                            <label htmlFor="pi_turnKeyBudget">TURN KEY BUDGET: </label>
                                        </MDBCol>
                                        <MDBCol md="4" sm="12" className={`${STYLES.selectCont}`}>
                                            <select id="pi_turnKeyBudget" name="pi_turnKeyBudget" onChange={e => formData.pi_turnKeyBudget = e.target.value}>
                                    ...
                                    <MDBRow className={STYLES.formRadio}>
                                        <MDBCol md="6" sm="12" className="mt-2">
                                            ...
                                            <input onClick={() => setHasPurchasedLand(true)} checked={hasPurchasedLand === true} type="radio" id="hasPurchasedLandYes" onChange={() => { }} />
                                            <label htmlFor="hasPurchasedLandYes">Yes</label>
                                            <input onClick={() => setHasPurchasedLand(false)} checked={hasPurchasedLand === false} type="radio" id="hasPurchasedLandNo" onChange={() => { }} />
                                ...
                                <section id="choose_your_design">
                                    ...
                                    <MDBRow className={STYLES.formCyd}>
                                        ...
                                        <MDBCol md="3">
                                            ...
                                            <select id="plan_1" name="plan_1" value={selectedPlan1.planCode} onChange={(e) => handleSelectedPlan(e, "plan_1")} required>
                                                <option value={''}>--none--</option>
                                                {homePlanList.map((hp, index) => (
                                                    <option value={hp?.planCode} key={index}>{hp.name}</option>
                                                ))}
                                            </select>
                                            {(selectedPlan1 && selectedPlan1.planCode) ?
                                                <div className={STYLES.imgCont}> <img className="disablecopy" src={selectedPlan1.imageUrl} /></div> :
                                                <div className={STYLES.noImgCont} ><img className="disablecopy" src='/images/common/no-img.png' /></div>}
                                        </MDBCol>
                                        <MDBCol md="3">
                                            <p className={STYLES.title}>Additional Study Set:</p>
                                            <select id="plan_2" name="plan_2" onChange={(e) => handleSelectedPlan(e, "plan_2")}>
                                                <option value={''}>--none--</option>
                                                {homePlanList.map((hp, index) => (
                                                    <option value={hp?.planCode} key={index}>{hp.name}</option>
                                                ))}
                                            ...
                                            {(selectedPlan2 && selectedPlan2.planCode) ?
                                                <div className={STYLES.imgCont}> <img className="disablecopy" src={selectedPlan2.imageUrl} /></div> :
                                                <div className={STYLES.noImgCont} ><img className="disablecopy" src='/images/common/no-img.png' /></div>}
                                        </MDBCol>
                                        <MDBCol md="3">
                                            <p className={STYLES.title}>Additional Study Set:</p>
                                            <select id="plan_3" name="plan_3" onChange={(e) => handleSelectedPlan(e, "plan_3")}>
                                                <option value={''}>--none--</option>
                                                {homePlanList.map((hp, index) => (
                                                    <option value={hp?.planCode} key={index}>{hp.name}</option>
                                                ))}
                                            ...
                                            {(selectedPlan3 && selectedPlan3.planCode) ?
                                                <div className={STYLES.imgCont}> <img className="disablecopy" src={selectedPlan3.imageUrl} /></div> :
                                                <div className={STYLES.noImgCont} ><img className="disablecopy" src='/images/common/no-img.png' /></div>}
                                        </MDBCol>
                                    </MDBRow>
                                    <MDBRow className="mt-2">
                                        <MDBCol md="12" sm="12"> Total Calculated Price : $ {total} CDN</MDBCol>
                                        <MDBCol md="12" sm="12"> (NOTE: Canadian residents add applicable taxes)</MDBCol>
                                    </MDBRow>
                                </section>
                                ...
                                <section id="payment_options">
                                    ...
                                    <MDBRow className={STYLES.formText}>
                                        <MDBCol md="5" sm="12" className="mb-2">
                                    ...
                                    <MDBRow className={STYLES.formText}>
                                        ...
                                        <MDBCol md="7" sm="12" >
                                            <input type="text" id="po_ccNumber" name="po_ccNumber" className={STYLES.inputBox} value={formData.po_ccNumber} onChange={handleInput} required />
                                        </MDBCol>
                                    </MDBRow>
                                    <MDBRow className={STYLES.formText} >
                                        <MDBCol md="5" sm="12">
                                            <label htmlFor="po_ccExpiration">EXPIRE: (M/Y)</label>
                                        </MDBCol>
                                        <MDBCol md="3" sm="12">
                                            <input type="text" id="po_ccExpiration" name="po_ccExpiration" className={STYLES.inputBox} value={formData.po_ccExpiration} onChange={handleInput} required />
                                        </MDBCol>
                                        <MDBCol md="4" sm="12">
                                            <MDBRow>
                                                ...
                                                <MDBCol md="6" sm="12" style={{ float: "left" }}>
                                                    <input type="text" id="po_3NoCode" name="po_3NoCode" className={STYLES.inputBox} value={formData.po_3NoCode} onChange={handleInput} required />
                                                </MDBCol>
                                                <MDBCol md="2"></MDBCol>
                                            </MDBRow>
                                ...
                                <section id="acknowledgement">
                                    <MDBRow className={`${STYLES.checkbox} ${STYLES.ackCont} my-3`}>
                                        <MDBCol md="5" sm="12"> </MDBCol>
                                        <MDBCol md="7" sm="12">
                                            <div className="d-flex align-items-start">
                                                <input className="mt-2 mr-2" onClick={() => setCopyRightAck(!copyRightAck)} checked={copyRightAck === true} type="checkbox" required id="copyRightAck" onChange={() => setCopyRightAck(!copyRightAck)} />
                                                <label htmlFor="copyRightAck">I understand that the Study Set is a copyrighted design.</label>
                                            </div>
                                        </MDBCol>
                                        <MDBCol md="5" sm="12"> </MDBCol>
                                        <MDBCol md="7" sm="12">
                                            <div className="d-flex align-items-start">
                                                <input className="mt-2 mr-2" onClick={() => setNoRefundAck(!noRefundAck)} checked={noRefundAck === true} type="checkbox" required id="noRefundAck" onChange={() => setNoRefundAck(!noRefundAck)} />
                                ...
                                <section id="complete_button">
                                    <MDBRow>
                                        <MDBCol md="12" className={`${STYLES.completeOrderContainer}`}>
                                            <MDBBtn type="submit" size="sm" className={`${STYLES.completeOrderBtn}`}>Complete Order</MDBBtn>
                                            <a href={homePlanFloorPlanUrl(planCode)}><MDBBtn type="button" size="sm" className={`${STYLES.completeOrderBtn}`}>&#8592; Home plan</MDBBtn></a>
                                        </MDBCol>
                                        <MDBCol md="12" className={`${STYLES.completeOrderSubtextCont}`}>
                                            <strong className={`${STYLES.completeOrderSubtext}`}>Your study set will be processed <br />
                        ...
                        <MDBCol md="5" sm="12" className={STYLES.descCont}>
                            ...
                            <h4>A Study Set is a 1/4" = 1'-0" scale professionally prepared plan.</h4>
                            <p>Each study set includes all four exterior elevations and two floor plans, drawn to standard construction scale, so that you can experiment with furniture placement and traffic patterns, before committing to a specific home design.</p>
                            <p>A Study Set does not contain the detailed construction specifications found in our full working blueprints and may not be used to build your home; they are stamped “NOT FOR CONSTRUCTION”.</p>
                            <p>Each Study Set is just $250.00 in Canadian funds. You’ll recoup the cost for a study sets when entering into a design agreement or construction contract.</p>
