import {
  HOME_PLAN_DETAILS_CONST,
  HOME_PLAN_DETAILS_PATH,
} from "../../common/constants/homePlans";

export const processHomePlans = (planCode, allHomePlans, category) => {
  const HOME_PLANS_NAVBAR = [
    {
      title: HOME_PLAN_DETAILS_CONST.FLOOR_PLAN_LABEL,
      href: `${HOME_PLAN_DETAILS_PATH.FLOOR_PLAN}/${planCode}`,
    },
  ];
  ...
  try {
    ...
    if (result.currHomePlan) {
      let currIndex = homePlans.reduce(
        (r, h, i) => (h.planCode === planCode ? (r.push(i), r) : r),
        []
      )[0];
      ...
      if (result.currHomePlan && result.currHomePlan.pictures === "x") {
        HOME_PLANS_NAVBAR.push({
          title: HOME_PLAN_DETAILS_CONST.IMAGE_GALLERY_LABEL,
          href: `${HOME_PLAN_DETAILS_PATH.IMAGE_GALLERY}/${planCode}`,
        });
      }
      if (result.currHomePlan && result.currHomePlan.quote === "x") {
        HOME_PLANS_NAVBAR.push({
          title: HOME_PLAN_DETAILS_CONST.PRICE_QUOTE_LABEL,
          href: `${HOME_PLAN_DETAILS_PATH.PRICE_QUOTE}/${planCode}`,
        });
      }
      if (result.currHomePlan && result.currHomePlan.study === "x") {
        HOME_PLANS_NAVBAR.push({
          title: HOME_PLAN_DETAILS_CONST.STUDY_SET_LABEL,
          href: `${HOME_PLAN_DETAILS_PATH.STUDY_SET}/${planCode}?scroll=false`,
          scroll: true,
        });
      }
    }
