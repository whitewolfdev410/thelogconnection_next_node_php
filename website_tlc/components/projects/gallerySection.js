...
export const ProjectGallerySection = ({ providedProjectCode }) => {

  const router = useRouter();
  let projectCode = providedProjectCode ? providedProjectCode : router.query.project;
                          ...
                          {a.subSectionItems.map((c, d) => (
                            <Carousel.Item key={d}>
                              <div className={STYLES.thumbnailCont}>
                                <img className='enablecopy' src={c.thumbnailUrl} alt={c.label} onClick={() => onClickItem(c)} />
                              </div>
