        ...
        <section className={STYLES.imageModal}>
            <Zoom>
                <div className={`${STYLES.thumbCont} ${C_STYLES.boxShadow1}`}>
                    <img className="disablecopy" onClick={() => toggle(props)} src={props?.imgThumbSrc} alt={props?.label}/>
                </div>
            </Zoom>
            <MDBModal isOpen={state.modal} toggle={toggle} size="lg">
                ...
                <MDBModalBody>
                    <img className="disablecopy" src={state?.item?.imgSrc} alt={state?.item?.label}/>
                </MDBModalBody>
            </MDBModal>
        </section>
    )
