...
import { getStillImages, getTechVids, getTimelapseVids } from "../../common/services/building-styles";
...
export const DasGallery = (props) => {
    ...
    return (
        <div>
            ...
            <MDBContainer className={`${STYLES.dasGallery} mb-5 py-5 px-2`}>
                <MDBRow>
                        ...
                        {activeItem.type === 'video' ?
                            <div className={`${STYLES.vidCont} ${ANIMATION.fadeIn}`}>
                                <Waypoint onEnter={handleEnterViewport} onLeave={handleExitViewport}>
                                    <span>
                                    <ReactPlayer controls={true} url={activeItem.url} className="react-player" playing={videoThumb.isPlaying} width="100%" height="100%" />
                                    </span>
                                </Waypoint>
                            </div>
                            :
                            <div className={`${STYLES.imgCont} ${ANIMATION.fadeIn}`} key={new Date()}>
                                <img  className="disablecopy" src={`${IMG_ROOT_SL}${activeItem.imgName}`} />
                            </div>}
                        <div className={`${STYLES.caption} text-center`}>{activeItem?.caption}</div>
                    </MDBCol>
                    <MDBCol className={`${STYLES.listCont} pb-3`} md="4">
                                ...
                                <div className={`${STYLES.timeLapse}`} >
                                    {timelapseVids.map((tv, i) => (
                                        <div className={`${STYLES.videoThumbCont} p-2`} key={i}>
                                            <img className="disablecopy" src={tv.thumbUrl} />
                                            <div className={STYLES.playButton} onClick={() => onClickPlayBtn(tv, videoThumb.isPlaying)} />
                                        </div>
                                    ))}
                                </div>
                            </div> : <></>}
                        {techVids && techVids.length > 0 ?
                            <div>
                                ...
                                <div className={`${STYLES.technical}`}>
                                    {techVids.map((tv, i) => (
                                        <div className={`${STYLES.videoThumbCont} ${STYLES.technical} p-2`} key={i}>
                                            <img className="disablecopy" src={tv.thumbUrl} />
                                            <div className={STYLES.playButton} onClick={() => onClickPlayBtn(tv, videoThumb.isPlaying)} />
                                        </div>
                                    ))}
                        ...
                        {stillImages && stillImages.length > 0 ?
                            <div>
                                <div className={`${STYLES.sectionLabel} pb-2`}>Still Images</div>
                                <Carousel
                                    arrowLeft={stillImages.length && stillImages.length > 12 ? <CarouselPrevBtn /> : (<></>)}
                                    arrowRight={stillImages.length && stillImages.length > 12 ? <CarouselNextBtn /> : (<></>)}
                                    cols={3}
                                    rows={2}
                                    gap={15}
                                    loop
                                    autoplay={10000}
                                    dot={customDot}
                                    showDots={stillImages.length && stillImages.length > 12 ? true : false}
                                    responsiveLayout={[
                                        {
                                            breakpoint: 767.98,
                                            cols: 6,
                                    ...
                                    {stillImages.map((g, i) => (
                                        <Carousel.Item key={i}>
                                            <div className={STYLES.thumb}>
                                                <img className="disablecopy" src={`${IMG_ROOT_SL}${g.imgName}`} onClick={() => onClickItem(g)} alt={g.caption} />
                                            </div>
                                        </Carousel.Item>
                                    ))}
                                </Carousel>
                                {stillImages.length && stillImages.length > 12 ?
                                    <div className={`${STYLES.viewMore} pb-3 pb-md-0`}>View More</div> : <></>
                                }
                            </div> : <></>}
                    </MDBCol>
                </MDBRow>
