...
export const getHomePlan = (planCode, filterParameters = {}) => {
    let queryParameters = Object.keys(filterParameters)
        .map(k => encodeURIComponent(k) + '=' + encodeURIComponent(filterParameters[k]))
        .join('&');

    let DATA_URL = `${process.env.PHP_ENDPOINT}?function=GetLogHomePlans&params=${planCode}&${queryParameters}`;
...
export const captureUserActivity = (payload) => {
    let DATA_URL = `${process.env.PHP_ENDPOINT}?function=CaptureUserActivity&params=''`;
    return postData(DATA_URL, payload);
}