...
const NewsletterEntryPage = () => {
  ...
  return (
    <>
      <Head>
        <title>Monthly News Letter by The Log Connection</title>
        <meta property="og:title" content={`Monthly News Letter by The Log Connection`} />
        <meta property="og:url" content={`${process.env.DOMAIN}/newsletter`} />
                ...
                <Link href={{ pathname: '/newsletter', query: { code: newsletter[0]?.NewsletterCd } }}>
                  <a href={`/newsletter?code=${newsletter[0]?.NewsletterCd}`}>
                    <div className={`${STYLES.imgCont}`}>
                      <img className="disablecopy" src={`${NEWSLETTER_COVER}${newsletter[0]?.Cover}`}></img>
                    </div>
            ...
            <div className={`${STYLES.newsletterCont} p-3`} key={i}>
              <Link href={{ pathname: '/newsletter', query: { code: n.NewsletterCd } }}>
                <a href={`/newsletter/${n.NewsletterCd}`}>
                  <img className="disablecopy" src={`${NEWSLETTER_COVER}${n.Cover}`}></img>
