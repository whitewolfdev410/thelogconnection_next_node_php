...
const TestimonialsPage = () => {
  ...
  return (
    <>
      <Head>
        <title>Log Home Owners Testimonials for The Log Connection</title>
        <meta property="og:title" content={`Log Home Owners Testimonials for The Log Connection`} />
        <meta property="og:url" content={`${process.env.DOMAIN}/testimonials`} />
                ...
                {DATA.map((t, i) => (
                  <MDBCol key={i} size="6" md="6" lg="6" sm="12" xl="6" xs="12" className="px-lg-5">
                    <div className={`${TLC_STYLES.thumbnail} my-3`} onClick={(e) => onClickItem(e, i, t)}>
                      <img src={t.imageUrl} className={(activeIndex === i ? TLC_STYLES.selected : "") + ' disablecopy'} />
                      <p>{t.label}</p>
                    </div>
                  </MDBCol>
                ))}
