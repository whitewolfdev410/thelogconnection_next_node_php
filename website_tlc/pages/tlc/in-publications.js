...
const InPublicationsPage = () => {
  ...
  return (
    <>
      <Head>
        <title>Log Home Publications for The Log Connection</title>
        <meta property="og:title" content={`Log Home Publications for The Log Connection`} />
        <meta property="og:url" content={`${process.env.DOMAIN}/in-publications`} />
      ...
      <section className={TLC_STYLES.inPublications}>
                  ...
                  <div className={`${TLC_STYLES.pagesCont} mb-5`}>
                    ...
                    {mag.pagesImgUrl.map((m, i) => (
                      <div key={i} className={`${TLC_STYLES.pages} m-2`}>
                        <img className="disablecopy" src={m} />
                      </div>
                    ))}
        ...
        <section className={TLC_STYLES.published}>
                ...
                <MDBCol key={i} onClick={() => toggle(m)}>
                  <div className={`${TLC_STYLES.magCont} my-0 text-center`}>
                    ...
                    <div className={`${TLC_STYLES.imgCont}`}>
                      <img className="disablecopy" src={m.thumbUrl} />
                    </div>
                  </div>
                </MDBCol>
              ))}
          ...
          <MDBModal isOpen={state.modal} toggle={toggle} className={TLC_STYLES.modal}>
            ...
            <MDBModalBody>
              {state?.item?.pages?.map((p, i) => (<img className="disablecopy" src={p} />))}
            </MDBModalBody>
          </MDBModal>
        </section>
      </section>
